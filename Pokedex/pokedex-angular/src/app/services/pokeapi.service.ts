...
export class PokeapiService {
  ...
  async getAllPokemon(): Promise<Pokemon[]> {
    ...
    for (const data of allDataRaw) {
      const pokemon: Pokemon = {
        ...
        types: (data.types as any[]).map((t: any) => t.type.name)
      };
  ...
  async getPokemonDetail(id: number): Promise<PokemonDetail> {
    const request: Observable<Object> = this.http.get(`${this.BASE_URL}pokemon/${id}`);
    const dataRaw: any = await lastValueFrom(request);
    const pokemon: PokemonDetail = {
      id: dataRaw.id,
      name: dataRaw.name,
      imageUrl: dataRaw.sprites.other['official-artwork'].front_default,
      types: (dataRaw.types as any[]).map((t: any) => t.type.name),
